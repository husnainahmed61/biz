 ...
 *                  For example: considering that a post can have multiple authors, a pivot table that connects two tables (users and posts) must be named posts_users and must have auction_id and user_id as identifying columns for the posts.id and users.id tables.
...
class MY_Model extends CI_Model
{
    ...
    */
    ...
    public function max_where($column, $conditions)
    {
        if (is_array($conditions)) {
            return $this->db->select_max($column)->get($this->table, $conditions)->result()[0]->$column;
        }
        return false;
    }
