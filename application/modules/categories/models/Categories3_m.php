 ...
 * Time: 6:51 PM
 */
class Categories3_m extends MY_Model
{
    ...
    public function __construct()
    {
        ...
        $this->table = $this->db->dbprefix('categories3');
        $this->primary_key = 'id';
...
//        print_r($lastQuery);
    ...
    public function getAuctionsByAttributes($currency = '', $ids = '', $values = '', $sort = NULL, $limit = NULL, $postType = '', $category3ID = '',$start_price_range='',$end_price_range='',$search='')
    {
        ...
        if (/*$sort == 1 && $ids!='' && $values!='' */TRUE) {
            ...
            $this->db->where("ssxa.category3_id", $category3ID);
    ...
    public function countAuctionsByAttributes($currency = '', $ids = '', $values = '', $postType = '', $category3ID = '',$start_price_range='',$end_price_range='',$search='')
    {
        $date_expire = date('Y-m-d H:i:s');
        if ($ids != '' && $values != '' && $postType != '' /*TRUE*/) {
            $this->db->select("COUNT(DISTINCT ssxa.id) AS count");
            $this->db->from("ssx_auction_attributes ssxaa");
            $this->db->join("ssx_auctions ssxa", "ssxa.id=ssxaa.auction_id", "inner");
            if (!empty($ids)) {
                $this->db->where_in("ssxaa.attribute_value_id", $ids);
            }
